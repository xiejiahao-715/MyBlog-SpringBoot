...
public class OssServiceImpl implements OssService {
    ...
    private OSS createOssClient(){
        return new OSSClientBuilder()
                .build(OssPropertiesUtil.END_POINT, OssPropertiesUtil.ACCESS_KEY_ID, OssPropertiesUtil.ACCESS_KEY_SECRET);
    }
    ...
    public void downloadFile(HttpServletResponse response,String path,String filename,String suffix) {
        ...
        // 判断文件是否存在
        boolean found = ossClient.doesObjectExist(OssPropertiesUtil.BUCKET_NAME,path);
        if(!found) throw new MyException("获取博客内容(文件不存在)");
        // ossObject包含文件所在的存储空间名称、文件名称、文件元信息以及一个输入流。
        OSSObject ossObject = ossClient.getObject(OssPropertiesUtil.BUCKET_NAME, path);
        try(OutputStream outputStream =response.getOutputStream();
            ...
            response.addHeader("Content-Length", String.valueOf(ossObject.getResponse().getContentLength()));
    ...
    public void downloadBlogZip(HttpServletResponse response, String path, String filename,String suffix) {
        ...
        try (BufferedReader blogReader = new BufferedReader(new InputStreamReader(ossObject.getObjectContent()));
            ...
            String imgTagRegex = String.format("(<img\\b.*?src=\")(%s|%s)/(([^\"]*?/)*(.*?))(\".*?>)", OssPropertiesUtil.FULL_DOMAIN, OssPropertiesUtil.PROXY_DOMAIN);
            // 匹配markdown语法中的图片的路径 例如 ![test.jpg](/resource/test.jpg) 匹配出test.jpg
            String imgMdRegex = String.format("(!\\[.*?\\])\\( *?(%s|%s)/(([^\\)]*?/)*(.*?))\\)", OssPropertiesUtil.FULL_DOMAIN, OssPropertiesUtil.PROXY_DOMAIN);
            ...
            for(String imagePath :imageFilePaths){
                OSSObject object = ossClient.getObject(OssPropertiesUtil.BUCKET_NAME,imagePath);
            ...
            response.setContentType("application/octet-stream");
            // 告知浏览器文件的大小
            response.addHeader("Content-Length", String.valueOf(tempZipFile.length()));
            ...
            e.printStackTrace();
            throw new MyException(ResultCode.SYSTEM_ERROR.getCode(), "解析并生成博客zip文件失败,可能是代码错误");
        } finally {
            ossClient.shutdown();
            try {
                if(inputStream != null){
    ...
    public boolean uploadFile(MultipartFile file, String path,ObjectMetadata metadata) {
        ...
        try {
            ...
            PutObjectRequest putObjectRequest = new PutObjectRequest(OssPropertiesUtil.BUCKET_NAME,path,inputStream);
    ...
    public boolean isFileExist(String path) {
        // 创建OSSClient实例。
        OSS ossClient = createOssClient();
        boolean isExist = ossClient.doesObjectExist(OssPropertiesUtil.BUCKET_NAME,path);
        ossClient.shutdown();
        return isExist;
    }
    ...
    public boolean deleteFile(String path) {
        ...
        try {
            ...
            ossClient.deleteObject(OssPropertiesUtil.BUCKET_NAME,path);
        } catch (Exception e){
            e.printStackTrace();
            return false;
        } finally{
